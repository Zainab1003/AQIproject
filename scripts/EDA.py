...
def calculate_aqi_single_pollutant(concentration, pollutant):
    ...
    # ===============Converting concentrations for specific pollutants ====================
    
    if pollutant == 'o3':
        # Converting µg/m³ to ppb: ppb = (µg/m³ × 24.45) / 48
        concentration = (concentration * 24.45) / 48
    elif pollutant == 'no2':
        # Converting µg/m³ to ppb: ppb = (µg/m³ × 24.45) / 46
        concentration = (concentration * 24.45) / 46
    elif pollutant == 'so2':
        # Converting µg/m³ to ppb: ppb = (µg/m³ × 24.45) / 64
        concentration = (concentration * 24.45) / 64
    elif pollutant == 'co':
        # Converting µg/m³ to ppm: ppm = (mg/m³) × 0.873
        concentration = (concentration / 1000) * 0.873

    # ==============Finding appropriate breakpoint ====================
...
def clean_air_quality_data(df):
    ...
    # Step2. Remove duplicate timestamps
    if 'datetime_utc' in df_clean.columns:
    ...
    # Step3. Handle negative values (pollutants can't be negative)
    ...
    # Step4. Handle extreme outliers using IQR method (per pollutant)
    ...
    for col in pollutant_cols:
        if col in df_clean.columns:
            ...
            if outliers > 0:
                print(f"    {col}: Capping {outliers} extreme outliers")
    ...
    # Step6. Remove AQI column if present (we'll recalculate it)
    if 'aqi' in df_clean.columns:
        print(f"\n Removing existing 'aqi' column (will be recalculated)")
...
def prepare_final_dataset(df):
    ...
    # Calculating individual pollutant AQIs and overall AQI
...
================================================================
This script performs  eda and uploads data to Hopsworks Feature Store
"""
...
def perform_comprehensive_eda(df, target='aqi'):
    ...
    for col in numeric_cols[:10]:  # Check first 10 numeric columns
        ...
        skewness[col] = skew
        if abs(skew) > 1:
            print(f"     {col}: Highly skewed (skewness={skew:.2f})")
    ...
    # Step6. Feature Importance (Statistical Tests)
...
def validate_dataframe_for_hopsworks(df):
    ...
    # Step6. Handle categorical columns
    ...
    # Step7. Handle object columns (ensure they're all strings)
    ...
    # Step8. Ensure numeric columns are float64 or int64
    ...
    # Step10. Check for infinity values
    numeric_cols = df_clean.select_dtypes(include=[np.number]).columns
    for col in numeric_cols:
    ...
    # Step11. Validate column names (no special characters except underscore)
